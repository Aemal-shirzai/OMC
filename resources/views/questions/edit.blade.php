...
<div class="" id="addPostParent">
	...
	<div id="formParent">
		...
		{!! Form::model($question,["method"=>"PUT","action"=>["QuestionController@update",$question->id],"files"=>"true","id"=>"postAddingForm"]) !!}
			...
			<div class="container text-center" id="PhotoParentDiv" @if($question->photos()->where('status',1)->first()) style="display: block;" @endif>
				...
				<div id="photoDiv">
					<img src="
						@if($question->photos()->where('status',1)->first())/Storage/images/questions/{{$question->photos()->where('status',1)->first()->path}}@endif" 
			...
			<div class="form-elements">
				...
				<a href="javascrip:void(0)" onclick="showTags()" id="addTagLink" class="btn  btn-sm {{($errors->has('tags') ? 'errorButton' : '')}}">
					<span id="tagsCount" class="badge badge-pill badge-light">{{$question->tags()->count()}}</span> &nbsp;
			...
			<div class="form-elements">
				<div class="dropdown-divider"></div>
				{!! Form::submit("Edit Question",["class"=>"btn btn-sm","id"=>"submitButton","onclick"=>"validatePostForm()"]) !!}
				<a href="{{route('questions.show',$question->id)}}" class="btn btn-sm" id="submitButton">Cancel<a>
			</div>
		{!! Form::close() !!}
	</div>
