...
<div class="" id="addPostParent">
	<h3 id="mainTitle">Edit your comment</h3>
	...
	<div id="formParent">
		...
		{!! Form::model($comment,["method"=>"PUT","action"=>["CommentController@update",$comment->id],"files"=>"true","id"=>"postAddingForm"]) !!}
			<div class="form-elements">
				...
				{!! Form::textarea("content",null,["class"=>"form-control postFormInputs". ($errors->has('content') ? ' formErrorForFields' : ''),"id"=>"crField","placeholder"=>"Add comment","maxLength"=>"65500","onkeyup"=>"validate_enable_button(this)"]) !!}
			...
			<div class="form-elements">
				...
				<div class="container text-center" id="PhotoParentDiv" @if($comment->photos()->where('status',1)->first()) style="display: block;" @endif>
					...
					<div id="photoDiv">
						<img src="
							@if($comment->photos()->where('status',1)->first())/Storage/images/comments/{{$comment->photos()->where('status',1)->first()->path}}@endif" 
							id="img-placeholder" class="img-fluid">
					</div>
				</div>
				{!! Form::file("photo",["class"=>"form-control","id"=>"crPhotoField","disabled"=>"true","style"=>"display:none;","onchange"=>"showAndcrValidateFile()"]) !!}
				...
				@if($comment->photos()->where("status",1)->first())
			...
			<div class="form-elements">
				{!! Form::submit("Edit Comment",["class"=>"btn btn-sm","id"=>"submitButton","onclick"=>"validateCommentReplyForm()"]) !!}
